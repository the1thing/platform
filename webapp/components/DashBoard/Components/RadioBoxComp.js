...
export default class RadioBoxComp extends Component {
  ...
  render() {
    let renderRadioList = () => {
      return this.props.radioList.map((value, key) => {
        return (
          <div
            key={key}
            className={this.renderClass(value)}
            onClick={e => {
              this.handleRadioClick(value);
            }}
          >
            <div className={this.renderRadioCircleClass(value)} />
            <div className={this.renderRadioTextClass(value)}>{value}</div>
          </div>
        );
      });
    };
    return <div className="radio-group-container">{renderRadioList()}</div>;
  }
}
