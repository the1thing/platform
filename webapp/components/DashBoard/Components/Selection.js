  ...
  render() {
    return (
      <div
        ...
        onClick={event => {
          this.props.disableClick ? event.preventDefault() : this.handleClick();
        }}
        ref={node => {
          this.node = node;
        }}
      >
        <div
          ...
          style={{ display: this.state.toggleVisiblity ? "block" : "none" }}
        >
        ...
        <div
          style={{
            color: "#030303",
            display: this.state.toggleVisiblity ? "none" : "block"
          }}
        >
        ...
        <div className="arrow-container">
          <div className={this.state.baseClass}>
            <div className="arrow-down-outer" />
    ...
    return (
      ...
      >
        ...
        {this.props.popupVisible && (
          <ul className="popover">{this.renderList()}</ul>
        )}
      </div>
    );
  }
  ...
  render() {
    return (
      <div className="subcomponent-spacing">
        <Row>
          <Col md={1} className="form-label" style={{ lineHeight: "48px" }}>
            {this.props.count}.
          </Col>
          <Col md={11}>
            <Selection
            ...
            <textarea
              ...
              className={
                this.props.error ? "Error-link-textares" : "about-link-textares"
              }
              style={{ width: "100%" }}
...
export class SelectMultiple extends Component {
    ...
    if (this.props.defaultValue.length > 0) {
      this.props.defaultValue.map((value1, key1) => {
        this.temp.map((value2, key2) => {
          if (value1 == value2) this.temp.splice(key2, 1);
        });
      });
      this.setState({
  ...
  renderSelectedList = () => {
    return this.state.selectedList.map((value, key) => {
      return (
        <div key={key} className="multiple-selection-button-container">
          <div className="multiple-selection-button">
            <span>{value}</span>
          </div>
          <div
            className="cross-image"
            onClick={() => this.handleRemoval(value, key)}
          />
        </div>
      );
    });
  ...
  render() {
    ...
    return (
      <div
        style={{ display: "flex", flexWrap: "wrap", width: this.props.width }}
      >
        {this.renderSelectedList()}
        <div
          className={this.state.mainContainer}
...
export class SelectMultipleBox extends Component {
    ...
    return this.state.selectedList.map((value, key) => {
      return (
        <div key={key} className="multiple-selection-button-container">
          ...
          <div
            className="cross-image"
            onClick={() => this.props.handleRemoval()}
          />
        </div>
      );
    });
  ...
  render() {
    let renderList = () => {
      return this.state.optionList.map((value, key) => {
        return (
          <li className="list" onClick={e => this.handleListClick(value, key)}>
            {value}
          </li>
        );
      });
      ...
      <div style={{ display: "-webkit-box", width: this.props.width }}>
        ...
        >
          ...
          {this.state.multiSelectPopupVisible && (
            <ul className="popover">{renderList()}</ul>
          )}
        </div>
      </div>
    );
